...
const desk_features_repository_1 = require("./desk.features.repository");
let DeskFeaturesService = class DeskFeaturesService {
    ...
    async findDeskFeatureById(desk_feature_id) {
    ...
    async addDeskFeature(addDeskFeatureDto) {
        const { desk_feature_name } = addDeskFeatureDto;
        const findDesk = await this.deskFeaturesRepository.findOne(desk_feature_name);
        if (findDesk) {
            throw new common_1.BadRequestException('Feature already exists');
        }
        else {
            const desk = this.deskFeaturesRepository.create({
                desk_feature_name: desk_feature_name,
            });
            try {
    ...
    async updateDeskFeature(updateDeskFeatureDto) {
        ...
        const desk = await this.findDeskFeatureById(desk_feature_id);
...
DeskFeaturesService = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(desk_features_repository_1.DeskFeaturesRepository)),
    __metadata("design:paramtypes", [desk_features_repository_1.DeskFeaturesRepository])
], DeskFeaturesService);
exports.DeskFeaturesService = DeskFeaturesService;
//# sourceMappingURL=desk.features.service.js.map